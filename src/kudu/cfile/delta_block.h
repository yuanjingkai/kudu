...
// Delta encoding supports signed integer types. (INT32, INT16, INT8)
//
...
class DeltaBlockBuilder : public BlockBuilder {
 public:
  ...
  int Add(const uint8_t *vals_void, size_t count) {
    ...
    while (!IsBlockFull(options_->block_size) && added < count) {
      ...
      if (cur_mblock_.num_elems++ == 0) {
        ...
        // Cast to the unsigned type first, so that integer overflow/underflow
        // is defined.
      ...
      if (cur_mblock_.num_elems < kEntriesPerMiniBlock) {
        ...
        uint32_t required_bytes =
            BitUtil::Ceil(cur_mblock_.bit_width * kDeltaEntriesPerMiniBlock, 8);

        // Compute the number of bytes needed to store the encoded values
  ...
  Slice Finish(rowid_t ordinal_pos) {
    ...
    for (size_t i = 0; i < num_mblocks; i++) {
      bit_writer_.PutValue(mblocks_[i].bit_width, 8);
    }
    // The frame of reference (minimum of the deltas).
    for (size_t i = 0; i < num_mblocks; i++) {
      bit_writer_.PutValue(mblocks_[i].min_delta, size_of_type * 8);
    }
    // Store the first value of every miniblock. So for random access,
    // we don't need to decode the deltas from the starting address
    // of the block.
    for (size_t i = 0; i < num_mblocks; i++) {
      bit_writer_.PutValue(mblocks_[i].first_elem, size_of_type * 8);
    }

    // Each block is made of miniblocks, each of them binary
    // packed with its own bit width.
    size_t k = 0;
    for (size_t i = 0; i < num_mblocks; i++) {
 ...
 private:
  ...
  uint64_t EstimateEncodedSize() const {
    return size_estimate_ + kOverheadPerMiniBlock;
  }
  ...
  struct DeltaMiniBlockState {
    // The number of elems. (also including the first_elem)
    size_t num_elems;
...
class DeltaBlockDecoder : public BlockDecoder {
  ...
  Status SeekAtOrAfterValue(const void *value_void,
    ...
    while ((cur_idx_ + kEntriesPerMiniBlock) < num_elems_) {
      if (target >= first_values_[cur_mblock_ + 1]) {
        ...
        // Stop here if the target < the first elem of
        // the next miniblock.
        break;
      }
    }
 ...
 private:
  Status DoGetNextValues(size_t *n_param, uint8_t *vals_void) {
    ...
    // If start index is not mini-block aligned, decode the valuse into
    // pending list first.
    if (k > 0) {
      if (pending_.size() == 0) {
        pending_.resize(kEntriesPerMiniBlock);
    ...
    while (n >= kEntriesPerMiniBlock) {
      // We always try to decode the entire miniblock at once.
  ...
  // Items that have been decoded but not yet yielded to
  // the user.
